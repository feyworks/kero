use crate::{Numeric, Vec4, Vec4F, impl_temp};
use kero_lua::{LuaModule, Temp};
use mlua::prelude::LuaResult;
use mlua::{FromLua, IntoLua, Lua, Value, Variadic};
...
macro_rules! impl_from_to {
    ($to:ident $to_fn:ident) => {
        impl FromLua for Vec4<$to> {
            #[inline]
            fn from_lua(value: Value, lua: &Lua) -> LuaResult<Self> {
                Vec4F::from_lua(value, lua).map(Vec4::$to_fn)
            }
        }

        impl IntoLua for Vec4<$to> {
            #[inline]
